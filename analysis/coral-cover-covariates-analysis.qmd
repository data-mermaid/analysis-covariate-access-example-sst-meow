...
Vector covariates such as MEOW are normally attached with [`attach_covariate_data()`](https://data-mermaid.github.io/mermaidr-covariates/reference/attach_covariate_data.html), which matches each sample event to the polygon containing it in a single call.

**Note (August 2026):** that route is temporarily unavailable here. The covariate parquet now uses DuckDB's native spatial geometry with CRS type modifiers, which requires DuckDB 1.5 or later; on R 4.3 the newest available Windows binary is DuckDB 1.2.1, which cannot read it. Until that is resolved, the chunk below joins against a local copy of the MEOW realm polygons stored in `data/realms.RData`. The intended covariate-based code is kept commented out so it can be restored once DuckDB 1.5 is available.

```{r extract-join-vector}
#| results: asis
# --- Preferred route, once DuckDB >= 1.5 is available -----------------------
# meow_covariate <- covariates_df %>%
#   filter(id == "meow_boundaries") %>%
#   pull(title)
#
# coral_meow <- coral_data %>%
#   attach_covariate_data(covariate = meow_covariate, columns = "REALM") %>%
#   rename(realm = REALM)
# ---------------------------------------------------------------------------

# Temporary fallback: join against the MEOW realm polygons stored locally.
load(here::here("data", "realms.RData"))   # provides `realms`, an sp object

meow_realms <- realms %>%
  st_as_sf() %>%
  st_set_crs(4326) %>%       # already WGS84; label it cleanly so st_join matches
  st_make_valid() %>%
  select(REALM)

# Turn off S2 - spherical geometry errors on these polygons
invisible(sf_use_s2(FALSE))

coral_meow <- coral_data %>%
  st_as_sf(coords = c("longitude", "latitude"), crs = 4326) %>%
  st_join(meow_realms, join = st_intersects) %>%
  # put latitude / longitude back as plain columns
...
- **Environmental covariates:** Accessed via the [`mermaidrcovariates`](https://data-mermaid.github.io/mermaidr-covariates/) package
  ...
  - Marine realms: Marine Ecoregions of the World (MEOW) classification. Currently read from a local copy in `data/realms.RData`; see the note above.
