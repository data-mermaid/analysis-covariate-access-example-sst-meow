...
#| results: asis
# Look the covariate up by its id so that a change to its title upstream
# cannot silently break this document.
meow_covariate <- covariates_df %>%
  filter(id == "meow_boundaries") %>%
  pull(title)

# `columns` currently accepts only ONE column - the package validates it with
# `col %in% ...` inside an `if()`, so passing a vector errors with "the
# condition has length > 1". Realm is the only MEOW field used here.
# MEOW column names are upper case; rename to match the rest of this document.
coral_meow <- coral_data %>%
  attach_covariate_data(
    covariate = meow_covariate,
    columns = "REALM"
  ) %>%
  rename(realm = REALM)
