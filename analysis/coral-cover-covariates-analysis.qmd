...
here::i_am("analysis/coral-cover-covariates-analysis.qmd")
library(tidyverse)
library(mermaidr)
library(plotly)
library(DT)
...
# The result holds one row per sample event, with each survey's daily series in
# a nested list-column. Unnest a single survey to show what that series is.
sst_example %>%
  slice(1) %>%
  ...
  datatable(
    ...
    caption = "Daily SST for one sample event, the 365 days before its survey date",
    rownames = FALSE
  ) %>%
  formatRound("value", digits = 2)
```

`summarise_zonal_statistics()` then reduces each of those series to a single value. The data is already downloaded, so this step is instant.
...
sst_example %>%
  ...
  datatable(
    ...
    caption = "Maximum SST for each of the 10 example sample events",
    rownames = FALSE
  ) %>%
  formatRound("value", digits = 2)
```
...
sst_data_file <- file.path(
  data_dir,
  paste0("coralMeowSst_", NumDays, "days_", Radius, "m.rds")
)

# Check if data already exists
if (file.exists(sst_data_file) & force_download == FALSE) {
  cat('<span style="color: #006400;">Loading previously downloaded SST data from: data/',
      basename(sst_data_file), '</span>\n\n')
  ...
  cat('<span style="color: #006400;">Data saved to: data/',
      basename(sst_data_file), '</span>\n\n')
}
```
