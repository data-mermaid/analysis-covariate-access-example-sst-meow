...
The next function of interest is [get_zonal_statistics](https://data-mermaid.github.io/mermaidr-covariates/reference/get_zonal_statistics.html), which can be used to extract raster data, like SST, from the covariates. In this case, we summarise SST across the 365 days prior to each sample date, as an example, but any window can be used via the `n_days` argument in the function.

```{r get-zonal-stats-standard}
# Start timer
start_time <- Sys.time()

coral_meow_sst <- coral_meow %>%
...
The global correlation between hard coral cover and SST (365 days prior to each survey) is `r round(overall_cor$estimate, 3)` (p = `r format.pval(overall_cor$p.value, digits = 3)`).
...
- **Environmental covariates:** Accessed via the [`mermaidrcovariates`](https://data-mermaid.github.io/mermaidr-covariates/) package
  - SST data: Daily Sea Surface Temperature over the 365 days prior to each survey date, within a 1 km radius
