...
mermaid_summ_ses_path <- file.path(dirname(here::here()), "data", "mermaid_summ_ses.rds")

if (file.exists(mermaid_summ_ses_path)) {
  mermaid_summ_ses <- readRDS(mermaid_summ_ses_path)
  cat('<span style="color: #4682B4;">Loaded cached summary sample events from', mermaid_summ_ses_path, '</span>\n\n')
} else {
...
#check full name of desired covariate
covariates_df %>% filter(id == "daily_dhw") %>% 
  select(title) #need full name of covariate

#set path, use if/else to read or cache the API call
covars_path <- file.path(dirname(here::here()), "data", "coral_data_dhw_30days.rds") 
...
nrow(dhw_4_or_more) #75 sites with DHW>4 in 30 days prior to survey
...
dhwAggHist <-
  plot_ly(data = dhw_binned,
          x = ~bin_left + bin_width / 2,  # plot at bin midpoint
          y = ~n,
          type = 'bar',
          width = bin_width * 0.9,
...
coral_data_with_covariate <- coral_data_with_covariate %>% 
  group_by(project, 
           ...
           sample_date,
           covariate) %>% 
  summarize(hard_coral_cover = mean(hard_coral_cover), 
            value = mean(value))

coral_data_with_covariate
